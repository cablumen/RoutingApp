...
namespace Samraksh.Components.Utility
{
	...
	public class EnhancedEmoteLCD 
	{
		...
		/// </summary>
		public LCD[] CurrentChars
		{
            get { return null; }
			/*get
			{
				var currChars = new LCD[_currentChars.Length];
				lock (_criticalSectionLock)
				{
					_currentChars.CopyTo(currChars, 0);
				}
				return currChars;
			}*/
		}
		private readonly LCD[] _currentChars;
		...
		public EnhancedEmoteLCD()
		{
			...
			// Initialize _currentChars
			_currentChars = new LCD[4];
			for (var i = 0; i < _currentChars.Length; i++)
			{
				_currentChars[i] = LCD.CHAR_NULL;
			}
		...
		public new bool WriteRawBytes(int data4, int data3, int data2, int data1)
		{
			...
			/*lock (_criticalSectionLock)
			{
				_currentChars[4 - 1] = (LCD)data4;
				_currentChars[3 - 1] = (LCD)data3;
				_currentChars[2 - 1] = (LCD)data2;
				_currentChars[1 - 1] = (LCD)data1;
